...
#Complete prebehavior script for sleeves with no roll and manipulators

import maya.cmds as mc

def patch():#Alejandro Munoz Medina
    ...
    def main():
        ...
        def wristNoTwist(side):

            import maya.cmds as mc

            # joint in question
            frArmTip = ("%s_arm_02_tip_twist" % side)
            ...
            frArmNtTip = mc.duplicate(frArmTip, n="%s_arm_02_tip_no_twist" % side)

            # refer to shoulder
            shoulder = ("%s_arm_01_twist" % side)

            # create blender
            blender = mc.createNode("blendTwoAttr", n="%s_wrist_twist_choice_bta" % side)

            # connect them
            mc.connectAttr("%s.twistY" % shoulder, "%s.ry" % frArmNtTip[0])
            mc.connectAttr("%s.ry" % frArmTip, "%s.input[0]" % blender)
            mc.connectAttr("%s.ry" % frArmNtTip[0], "%s.input[1]" % blender)
            mc.connectAttr("%s.%s_nr" % (attributeCtrl, side), "%s.ab" % blender, f=True)

        # forearm

        def forearmNoTwist(side):

            import maya.cmds as mc

            # joint in question
            frArm = ("%s_arm_02_bend_twist" % side)

            # refer to attribute control
            attrControl1 = ("%s_arm_SW_ctrl" % side)

            # create the no twist joint
            frArmNt = mc.duplicate(frArm, n="%s_arm_02_bend_no_twist" % side)

            # refer to shoulder
            shoulder1 = ("%s_arm_01_twist" % side)

            # create blender
            blender1 = mc.createNode("blendTwoAttr", n="%s_forearm_twist_choice_bta" % side)

            # connect them
            mc.connectAttr("%s.twistY" % shoulder1, "%s.ry" % frArmNt[0])
            mc.connectAttr("%s.ry" % frArm, "%s.input[0]" % blender1)
            mc.connectAttr("%s.ry" % frArmNt[0], "%s.input[1]" % blender1)
            mc.connectAttr("%s.%s_nr" % (attrControl1, side), "%s.ab" % blender1, f=True)

        # elbow (front)

        def elbowFrontNoTwist(side):

            import maya.cmds as mc

            # joint in question
            elbowFrnt = ("%s_arm_02_base_twist" % side)

            # refer to attribute control
            attrControl2 = ("%s_arm_SW_ctrl" % side)

            # create the no twist joint
            elbowFrntNt = mc.duplicate(elbowFrnt, n="%s_arm_02_base_no_twist" % side)

            # refer to shoulder
            shoulder2 = ("%s_arm_01_twist" % side)

            # create blender
            blender2 = mc.createNode("blendTwoAttr", n="%s_elbowBack_twist_choice_bta" % side)

            # connect them
            mc.connectAttr("%s.twistY" % shoulder2, "%s.ry" % elbowFrntNt[0])
            mc.connectAttr("%s.ry" % elbowFrnt, "%s.input[0]" % blender2)
            mc.connectAttr("%s.ry" % elbowFrntNt[0], "%s.input[1]" % blender2)
        ...
        def topPivotOrient(number):
            ...
            pivotControl = ("R_sleeve_0%s_pivot_top_ctrl" % number)
            sleeveControl = mc.listRelatives(pivotControl, c=True, type="transform")
            pivotControlTopGrp = mc.listRelatives("%s_orig" % pivotControl, p=True, )

            mc.parent(sleeveControl[0], w=True)

            mc.parent("%s_orig" % pivotControl, w=True)

            mc.setAttr("%s_orig.scale" % pivotControl, -1, -1, -1)

            mc.parent(sleeveControl, pivotControl)
        ...
        def rightSideOrient(number):
            ...
            controller = ("R_sleeve_0%s_ctrl" % number)
        ...
        def fix5thRing(side):
            ...
            ring = ("%s_sleeve_05_ctrl" % side)

            mc.scale(.75, .75, .75, "%s.cv[0:9]" % ring)

        # reparent viz controls for the sleeves

        def vcReparent(side):

            import maya.cmds as mc

            ctrl = ("%s_sleeve_top_pivot_attr_ctrl_orig" % side)

            mc.parent(ctrl, "%s_arm_02_no_twist_SKN_04_orig" % side)
        ...
        def parentManips(side):
            ...
            elbow = mc.duplicate("%s_arm_02" % side, n="%s_arm_02_no_twist" % side, po=True)
        ...
        mc.parent("R_sleeve_01_ctrl_orig", w=True)
        mc.setAttr("R_sleeve_01_pivot_top_ctrl_orig.scale", -1, -1, -1)
        mc.parent("R_sleeve_01_ctrl_orig", "R_sleeve_01_pivot_top_ctrl")
        mc.move(0, 1.351013, 0, "R_sleeve_01_pivot_top_ctrlShape.cv[0]", r=True)
        mc.move(0, 1.351013, 0, "R_sleeve_01_pivot_top_ctrlShape.cv[2:3]", r=True)
        mc.move(0, 1.351013, 0, "R_sleeve_01_pivot_top_ctrlShape.cv[5:9]", r=True)
