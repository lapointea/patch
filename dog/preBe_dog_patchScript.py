...
def patch():
    ...
    for side in sides :
        ...
        mc.setAttr('%s.translateZ' %ballIKCtrlOrig, (mc.getAttr('%s.translateZ' %ballIKCtrlOrig))*2)
    ...
    ## --- add blend Attribute for relaxPaw
    toesRelaxAttr = 'relaxPaw'
    relaxNode = 'leg_IK_ctrl'
    prefixes = ['front','back']
    for side in sides :
        for prefix in prefixes:
            toesRelaxNode = '%s_%s_%s' %(side,prefix,relaxNode)
            
            mc.addAttr( toesRelaxNode , ln = toesRelaxAttr , at = 'double' , min = 0 , max = 1 , dv = 0  )
            mc.setAttr('%s.%s' %(toesRelaxNode,toesRelaxAttr) , edit = True , k = True )  
    '''
    for toe in toesList :       
        ...
        placeNode = mc.rename(placeNode,'%s_relax' %toe )   
    ...
    targetList = ['back_paw_fk_01_orig','back_paw_fk_02_orig','front_paw_fk_01_orig','front_paw_fk_02_orig','front_paw_fk_03_orig']
    ...
    pawsReposeSkinSet = 'paws_repose_skin_set'
    pawsReposeAnimZsSet = 'paws_repose_ZS_set'
    pawsReposeAnimSet = 'paws_repose_set'

    pawReposeSkin = []
    pawReposeAnim = []
    pawReposeAnimZs = []   

    listpawReposeSets = mc.ls('*paw*', type='objectSet')  
    for set in listpawReposeSets:
        strippedSkin = set.rsplit('_skin_',1)[-1]
        strippedAnimZs = set.rsplit('_ZS_',1)[-1]
        
        if strippedSkin == 'set':
            pawReposeSkin.append(set)
        elif strippedAnimZs == 'set':
            pawReposeAnimZs.append(set)
        else:
            pawReposeAnim.append(set)

    mc.sets(pawReposeSkin,n=pawsReposeSkinSet)
    mc.sets(pawsReposeSkinSet,add='skin_set')
    for set in pawReposeSkin:
        mc.sets(set,rm='skin_set')
    mc.sets(pawReposeAnimZs,n=pawsReposeAnimZsSet)
    mc.sets(pawsReposeAnimZsSet,add='ALL_ZS_set')
    for set in pawReposeAnimZs:
        mc.sets(set,rm='ALL_ZS_set')
    mc.sets(pawReposeAnim,n=pawsReposeAnimSet)
    mc.sets(pawsReposeAnimSet,add='anim_set')
    for set in pawReposeAnim:
        mc.sets(set,rm='anim_set')    
    ...
    ###############################################################################################################################################
    # --- creates volumeSurf and deforms for neck //temporary use of firstSurf

    # --- ears smart Rig
    smartParentTarget = 'neck_ik_root'
    neckNoXformGrp = 'neck_noXformGrp'        
    ## --- create noXform Grp for ears

    rigPart = 'neck'
    deformGrp = 'ctrl_DeformGrp'
    endSurfDeformGrp = 'neck_endSurf_%s' %deformGrp

    # --- assign values for columeSurf characteristics
    volspansV = 12
    volspansU = 4
    volDeg = 3
    ## ---creates neck volSurfaces
    neckCurveGUIDEList = ['topNeck_curve_GUIDE','topMidNeck_02_curve_GUIDE','topMidNeck_01_curve_GUIDE','midNeck_curve_GUIDE',
                         'botMidNeck_01_curve_GUIDE','botMidNeck_02_curve_GUIDE',u'botNeck_curve_GUIDE']
    # --- create volume surface
    duplicatedCurves = mc.duplicate(neckCurveGUIDEList)
    loftCurves=[]
    mc.parent(duplicatedCurves , w=True)
    for u , curve in enumerate(duplicatedCurves):
        curveName = str(curve)
    ...
    mc.rebuildSurface(volSurf,rt=0,su=volspansU , sv=volspansV , kc=True , ch=False)
    mc.delete(loftCurves)
    mc.parent(volSurf,neckNoXformGrp)
    ...
    mc.setAttr('%s.turnOnPercentage' %tempPOSI,True)
    mc.connectAttr('%s.worldSpace[0]' %endSurf , '%s.inputSurface' %tempPOSI )
    volumeNeckDeformSKNList = []
    for i in range(0,UDeformsCount):
        for j in range(0,VDeformCount-1):    
            ## --- creates deform ctrl/skin hierarchy
            mc.select(cl=True)
            deformSkn = mc.joint(n='%s_deform_U%02d_V%02d_SKN' %(rigPart,i,j) )
            mc.setAttr('%s.radius' %deformSkn , 0.03)
            mc.select(cl=True)
            deformCtrl = mc.joint(n='%s_deform_U%02d_V%02d_ctrl' %(rigPart,i,j) )
            mc.setAttr('%s.radius' %deformCtrl , 0.03)
            mc.setAttr('%s.drawStyle' %deformCtrl,2)
            smartConnect(deformCtrl,deformSkn)
            shapes.create( deformCtrl, shape= 'pyramidDouble', size= 0.15, scale= [1, 1, 1 ], axis= 'y', twist= 0, offset= [0, 0, 0 ], color= [0.5,0,0.5] , colorDegradeTo= None, replace= True, middle= False)        
            deformOrig = orig.orig(objlist=[deformCtrl], suffix=['_orig'], origtype='joint', fromsel=False, viz=False, get_ssc=False, rtyp='auto')[0]
            mc.parent(deformSkn,deformOrig)
            volumeNeckDeformSKNList.append(deformSkn)
            ## --- getWorld Position placement for deform orig from UV
            UValue = ((1.000/(UDeformsCount-1))*i)
            VValue = ((1.000/(VDeformCount-1))*j)                      
            mc.setAttr('%s.parameterU' %tempPOSI , UValue)
    ...
    # ---  change parening of scapulas andleg_root_offsets
    bodyGimball = 'body_gimbal_ctrl'
    hips = 'hips_ctrl'
    chest = 'chest_ctrl'
    
    scapulasOffsetsOrig = ['L_scapula_offset_ctrl_orig','R_scapula_offset_ctrl_orig']
    ...
    mc.parent(scapulasOffsetsOrig,scapulasOrig)
    mc.parent(legOffsetsOrig,legsOffsetsOrig)

    ## --- TempCleanup of paws sets
    volumeNeckSkinSet = 'volume_neck_skin_set'
    mc.sets(volumeNeckDeformSKNList,n=volumeNeckSkinSet)
    mc.sets(volumeNeckSkinSet,add='skin_set')  
    ## --- add attribute for stubSkinny
    skinnyAttr = 'skinny'
    skinnyNode = 'traj'       
    mc.addAttr( skinnyNode , ln = skinnyAttr , at = 'double' , min = 0 , max = 1 , dv = 0  )
    mc.setAttr('%s.%s' %(skinnyNode,skinnyAttr) , edit = True , k = True )
    ## --- reparent stuff from baseNeck to accomodate volumeSurface rigPart
    neckDeformGrp = 'neckDeformGrp'
    neckDeformGrpChilds = mc.listRelatives(neckDeformGrp,c=True)
    neckDeformList = []
    for deform in neckDeformGrpChilds:
        splitName = deform.rsplit('_',1)[-1]
        if not splitName == 'tamp':
            neckDeformList.append(deform)  
    for deform in neckDeformList:
        mc.parent(deform,neckNoXformGrp)
    ...
    #--------------------------------------------------------------------------------------------------------------------make neck deform creation and attach
    ...
    #-------------------------------------------------------------------------------------------------------------------------make ears deform creation and attach
    ...
    #----------------------------------------------------------------------------------------------------------------------------add backLegs root helpers, same as earsRoot. reparent in cns group for pelvis
